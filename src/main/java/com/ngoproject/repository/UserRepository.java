...
public interface UserRepository extends JpaRepository<NGOUser, Integer> {
	...
	@Modifying
	@Transactional
	@Query(value = "update user set firstname=?1 , lastname =?2, email=?3,role=?4 where userid =?5", nativeQuery = true)
	void updateUserById(String fname, String lname, String email, String role , Integer id);
